...
async function getAllExperiments(): Promise<Map<string, ExperimentAssignment<any>>> {
    const apiToken = Xavier.getApiToken();
    const applicationId = Xavier.getApplicationId();

    if (!apiToken) {
        throw new Error("API token is not configured. Please call Xavier.configure() with a valid token.");
    }
...
async function getOneExperiment<T>(experimentId: string, defaultValue: T): Promise<T> {
    try {
...
export function useExperiment<T = boolean>(experimentId: string, defaultValue: T): SWRResponse<T> {
    if (!Xavier.configured()) {
        throw new Error("API token is not configured. Please call Xavier.configure() with a valid token.");
    }

    return useSWR(`assignments`, () => getOneExperiment<T>(experimentId, defaultValue));
}